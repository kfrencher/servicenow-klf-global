...
- `u_name`: The name of the import. This is used to identify the import. This name is derived from the scope name given to the KLF_RecordImporter constructor
- `u_state`: The current state of the import process. This is set to "loading" when the import is in progress and "complete" when the import is complete.
- `u_data_manifest`: A list of all the tables included in the import and the number of records imported for each table. This list is initialized at the start of the import and not updated.
- `u_loaded_data`: A list of all the records that have been imported. This list is updated in real-time during the import process.
- `u_loaded_data_json`: A JSON version of the `u_loaded_data` column that is easier to query.
- `u_checkpoint`: A JSON object that stores the checkpoint information for each table being imported. This is used to keep track of the last imported record for each table and to only import new or updated records in subsequent runs.
- `u_user_mapping`: A JSON object that stores the mapping information for users. This is used to map users from the source instance to users in the target instance.
- `u_group_mapping`: A JSON object that stores the mapping information for groups. This is used to map groups from the source instance to groups in the target instance.
...
- `u_table_name`: The name of the table the record will be imported to
- `u_record_sys_id`: The sys_id of the record on the source instance. This is used for updates and to prevent duplicates.
- `u_record_sys_updated_on`: The sys_updated_on value of the record on the source instance. This is used for updates.
- `u_record_xml`: The XML representation of the record. This is used to insert the data into the target table using global.GlideUpdateManager2
```javascript
...
const importer = new global.KLF_RecordImporter(connectionConfig, scopeName, import_log);	
```
